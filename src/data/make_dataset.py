...
def main():
    ...
    path_file = ((path_output / 'dataset.hdf5').resolve())
    path_file.unlink(missing_ok=True)  # delete file if exists
    file = h5py.File(path_file, 'a')
    for patient in tqdm(patient_list):

        image, mask = parse_image(patient, path_data_nii, path_mask_lung_nii)

        file.create_group(f"{patient}")
        file.create_dataset(f"{patient}/image", data=image, dtype="float32")
        file.create_dataset(f"{patient}/mask", data=mask, dtype="uint16")
